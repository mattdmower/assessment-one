...
function bWords(words) {
  const result = words.filter(word => word.startsWith('b') || word.startsWith('B'));
  return result;
}
...
function extend(originalArray, additionalItems) {
  const extendedArray = originalArray.concat(additionalItems);
...
function itemsWithLength(items, length) {
  const filteredItems = items.filter(item => item.length === length);
...
function smallestNItems(items, n) {
  ...
  const result = sortedItems.slice(0, n).reverse();
  return result;
}
